...
export interface TableItemSiswaProps {
  data: BiodataSiswa;
}

export const TableItemSiswa = React.memo(({ data }: TableItemSiswaProps) => {
    ...
    return (
      <div className="flex gap-2">
        ...
        <div className="">
          ...
          <p className="text-xs mb-1">{`${d?.kelas?.namaKelas || "Belum ada kelas"} | ${d?.user?.sekolah?.namaSekolah || "Belum ada sekolah"}`}</p>
          <p className="text-xs mb-2 text-gray-600">{`${d?.user?.nis || "-"} / ${d?.user?.nisn || "-"}`}</p>
          {badge && (
            <>
