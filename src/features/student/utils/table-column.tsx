import { Avatar, AvatarFallback, AvatarImage, Badge, Button, cn, dayjs, lang, simpleEncode, Tooltip, TooltipContent, TooltipProvider, TooltipTrigger } from "@/core/libs"
import {
  BiodataSiswa,
    ...
    ? [
        {
          ...
          cell: ({ row }) => {
            const status = row.original.statusKehadiran || row.original?.user?.status || '-';
            return (
              <div
              ...
              >
                {status === 'hadir' ? lang.text('attend') : status === 'belum hadir' ?  lang.text('notYetIn') : status}
              </div>
            );
          },
        },
        {
          ...
          cell: ({ row }) => (
            <Button
              className={(row.original.statusKehadiran || row.original?.user?.status) !== 'hadir' ? 'bg-green-200 text-[#0f4d3f] font-bold hover:bg-green-300' : 'bg-white/50 text-black/80 font-bold cursor-default hover:bg-whit/50'}
              onClick={() => handleAttend(row.original?.userId)}
              disabled={(row.original.statusKehadiran || row.original?.user?.status) === 'hadir'}
            >
              {lang.text('attend')}
            </Button>
          ),
