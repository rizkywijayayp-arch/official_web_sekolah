      ...
      <div className="max-w-7xl mx-auto px-4">
            ...
            <AnimatePresence mode="wait">
              {(activeTab === "daily" ? data.dailyEarlyBirds : data.monthlyChampions).map((siswa, idx) => (
                <motion.div
                  ...
                  className="relative group bg-white rounded-[2.5rem] p-6 border border-slate-200 shadow-sm hover:shadow-xl transition-all overflow-hidden"
                >
                  {/* Badge Peringkat */}
        ...
        {!loading && (activeTab === "daily" ? data.dailyEarlyBirds.length : data.monthlyChampions.length) === 0 && (
          <div className="text-center py-20 bg-white rounded-[3rem] border-2 border-dashed border-slate-200">
             <p className="text-slate-400 font-bold">Belum ada data tersedia untuk periode ini.</p>
          </div>
        )}
      </div>
    </section>
